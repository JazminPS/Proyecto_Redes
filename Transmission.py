...
for i in range (1,6*1330000 + 1):
    if ta < tsim:
        ...
        Package = np.append(Package,[[packageId,node,grade,ta,0]],axis=0)
...
def lostPackages(Package,lostpackages):
    for i in range(0,packageId+1):
        if Package[i,4] == -1:
            lostpackages[int(Package[i,2])] = lostpackages[int(Package[i,2])] + 1
    return lostpackages

#Nodos que llegaron al sink
#Retardo source to sink
def Latency(Package,latency):
    for i in range(0,packageId+1):
        if Package[1,4] != 1 and Package[1,4] != 0:
            latency[int(Package[i,2])] = latency[int(Package[i,2])] + (Package[i,3] - Package[i,4])
    return latency

#Gráficas
grades = [0,1,2,3,4,5,6]
lostpackages = lostPackages(Package, lostpackages)
latency = Latency(Package, latency)

plt.figure(1)
plt.plot(grades,lostpackages)
plt.xlabel('Grades')
